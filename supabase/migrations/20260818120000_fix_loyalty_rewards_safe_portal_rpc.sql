...
-- Correcao posterior a 20260817173000: mantem a evolucao do cartao fidelidade,
-- mas recompila as RPCs do Portal com guardas explicitas e grants finais.
...
declare
  normalized text;
begin
  select public.loyalty_normalize_service_name(
    nome_servico || ' ' || coalesce(categoria, '') || ' ' || coalesce(observacao, '')
  )
  into normalized
...
begin
  ...
  values (
    ...
    -10,
    'reward_redeem',
    'Cartao fidelidade completo: credito de Limpeza de manutencao e protecao gerado',
    'Sistema'
  );
...
begin
  ...
  foreach service_id in array service_ids loop
    if public.loyalty_is_eligible_own_service(service_id) then
      ...
      values
        (new.cliente_id, 1, 'own_service', new.id, 'Marcacao automatica por limpeza elegivel concluida', 'Sistema')
...
begin
  ...
  clean_notes := left(
    regexp_replace(trim(coalesce(p_observacoes, 'Agendado pelo Portal do Cliente')), '[[:cntrl:]]', ' ', 'g'),
    1000
  );
