-- Migration: corrige leitura de agenda JSONB string na RPC de recorrencia.
-- Nao altera dados em configuracoes_empresa; normaliza somente dentro da funcao.
...
BEGIN
    ...
    IF v_agenda_raw IS NULL THEN
        RAISE EXCEPTION 'Configuracao de agenda invalida';
    ELSIF jsonb_typeof(v_agenda_raw) = 'object' THEN
        v_agenda := v_agenda_raw;
    ELSIF jsonb_typeof(v_agenda_raw) = 'string' THEN
        BEGIN
            v_agenda := (v_agenda_raw #>> '{}')::jsonb;
