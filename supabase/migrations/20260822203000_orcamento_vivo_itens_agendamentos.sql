-- Orçamento vivo: rastreabilidade item -> agendamento.
-- Migration aditiva, sem backfill destrutivo e compatível com orçamentos antigos.
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'orcamento_itens_status_check'
          AND conrelid = 'public.orcamento_itens'::regclass
    ) THEN
        ALTER TABLE public.orcamento_itens
            ADD CONSTRAINT orcamento_itens_status_check
...
ALTER TABLE public.agendamentos
    ...
    ADD COLUMN IF NOT EXISTS orcamento_item_ids UUID[] NOT NULL DEFAULT '{}';

CREATE INDEX IF NOT EXISTS idx_orcamento_itens_agendamento
    ON public.orcamento_itens (agendamento_id);
CREATE INDEX IF NOT EXISTS idx_orcamento_itens_status
    ON public.orcamento_itens (orcamento_id, status);
CREATE INDEX IF NOT EXISTS idx_agendamentos_orcamento
    ON public.agendamentos (orcamento_id);
